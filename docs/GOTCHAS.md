...
## 1-2. 긴 오디오를 한 요청으로 전사시키면 모델이 뭉치고 잘라먹는다 (2026-07-30)

**증상**: 대면 면접 리포트에서 면접관/지원자가 잘못 갈리고 여러 발화가 한 덩어리로 합쳐진다.
"화자 분리가 목소리 기반이 아닌 것 같다"고 보이지만 — 실제로는 음향 분리가 **돌고 있었다**.

**진단 방법**: `interview_transcript_segments.speaker_label` 을 보면 어느 경로인지 바로 안다.
`화자1`/`화자2` + `start_ms` 존재 = Gemini 오디오 재전사(음향 기반). `candidate`/`interviewer` =
라이브 Web Speech 초안(문맥 기반). 운영 실측 2건 모두 전자였는데도 품질이 무너져 있었다:

| 녹음 | 결과 |
|---|---|
| 42분 | **61턴**으로 뭉침(한 턴 238초/1136자), 타임스탬프 전부 1000ms 라운드에 마지막 end 가 **실제의 1.8배**(4610초 vs 2547초) = 시각 환각 |
| 30분 | 469턴으로 세밀했으나 마지막 end 1423초 = **뒤 22% 통째 누락** |

**왜 치명적인가**: 역할 배정(`finalize`)은 **라벨 단위 일괄 UPDATE** 다. 뭉친 턴 하나에 두 사람이
섞이면 그 턴 전체가 한 역할로 박히고, 내용 기반 배정으로도 되돌릴 수 없다. 라벨 파편(`화2`, `3`)이
섞여 같은 사람이 별개 화자로 갈라지기도 했다.

**대응**: 라이브는 8분 파트로 나눠 파트별로 전사한다(`AudioPart`, 워커 한 실행 = 한 파트).
라벨은 `normalizeSpeakerLabel` 이 숫자만 뽑아 `화자N` 으로 정규화. 상세: LIVE_INTERVIEW_PLAN.md Phase 3-B.
**업로드 모드(사용자가 올린 단일 파일)는 분할이 안 되므로 40분 넘는 파일에 이 위험이 남아 있다.**
...
- **①의 예외 — 사내 도메인 오배달은 폴백한다**: 회사 메일 서버는 expernet.co.kr·erop.co.kr·
  intervia.kr 을 **로컬 가상 도메인**으로 취급해 외부로 릴레이하지 않는다. 그래서 그 도메인의
  주소가 실제로 존재해도 서버의 virtual mailbox table 에 없으면
  `550 5.1.1 … User unknown in virtual mailbox table` 로 튕긴다 — 겉보기는 수신자 거부지만
  원인은 발신 서버 쪽이고, 대체 경로로 보내면 MX 를 정상 조회해 배달된다
  (`isLocalDomainMisroute`). 이 경우엔 **장애 창을 열지 않는다** — 서버는 멀쩡하고 그 주소만
  튕긴 것이라, 나머지 메일은 계속 기본 경로로 나가야 하고 대체 경로 한도도 아껴야 한다.
  550 은 일시 오류가 아니므로 재시도 없이 **즉시** 폴백한다. 통지는 전용 문구
  `📬 사내 도메인 주소를 발신 서버가 거부 → 대체 경로로 발송` (주소별 6시간당 1회 — 담당자가
  어느 계정을 메일 서버에 등록해야 하는지 알아야 하므로 전역이 아니라 주소별). 이 에러가 뜨면
  수신자는 반드시 저 3개 도메인 중 하나다 — 외부 후보자(naver/gmail 등) 주소는 해당 없다.
