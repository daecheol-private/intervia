...
### 5.7 법인 면접 장소 주소 (`/api/orgs/me/addresses`)
- [x] **TC-5.7.1** 오프라인 면접 주소 저장/조회, 일정 제시 시 자동 채움
  > ✅ PUT → office_address/detail 저장 확인. 일정 자동채움은 §14에서.
  > ⚠️ 2026-07-31 다건(`org_addresses`)으로 대체 — 구 `PUT /api/orgs/me/address` 는 삭제됨.
- [ ] **TC-5.7.2** 주소 추가·목록·삭제 (org_admin) / member 는 조회만, 추가·삭제 403
- [ ] **TC-5.7.3** 같은 주소 재등록 → 새 행 대신 기존 행 반환(`duplicated: true`), 21개째 400
- [ ] **TC-5.7.4** 타 법인 주소 id 로 DELETE → 404 (테넌트 격리)
- [ ] **TC-5.7.5** 일정 제시 모달 — 저장된 주소 선택 / "다른 장소 직접 입력" 시 그 주소도
      주소록에 자동 추가(제시·직접 확정 양쪽). 같은 주소+상세면 중복 추가 안 함

**결과 요약(5):** 29 / 30 통과 + 1 SKIP (TC-5.5.2 실 Zoom 자격증명). (문서 표기 33은 추정치, 실제 30케이스)
※ TC-5.7.2~5 는 2026-07-31 다건 주소 도입분 — 미검증.
