...
- **인터랙티브 가이드(게임 튜토리얼식 둘러보기)** — 정적 안내를 넘어 실제 화면에서 대상 요소를 스포트라이트(딤+구멍) + 글로우 링 + 말풍선으로 짚어 준다. 페이지 진입 시 자동 노출(아래 "페이지 진입 가이드").
  - 정의: `lib/tour-scenarios.ts` (**3 시나리오** = 공고 페이지[`member-job-page`, 7스텝] / 후보 페이지[`member-candidate-page`, 4스텝] / 법인 설정[`org-settings`, 4스텝, org_admin 전용]). 각 스텝은 `path`(+hash, `{jobId}`/`{candidateId}` 치환) + `target`(CSS 선택자, 페이지의 `data-tour="..."` 속성) + 말풍선 문구 + 선택적 `presets`(이동 전 localStorage 세팅) + 선택적 `popup`(`{open, close?}` 셀렉터 — 드로어/모달/탭을 가이드가 직접 열고 닫으며 따라하게 함).
