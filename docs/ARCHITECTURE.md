...
- **온보딩 = 페이지 진입 가이드로 통일** (2026-06-25): 이전의 대시보드 순차 SetupGuide(0공고 hero / 진행 strip / 플로팅 `SetupGuideWidget` + `setup-progress`·`tour-targets` API + `lib/setup-steps.ts` + `culture-fit`/`job-create`/`resume-upload`/`ai-interview` 시나리오)는 **전부 제거**했다. **법인담당자(org_admin)·면접관(member) 모두** 아래 "페이지 진입 가이드"를 동일하게 쓴다(역할 구분 없음). 대시보드는 0공고일 때도 일반 화면(빈 KPI/목록 + 환영 헤더)만 보여준다. 법인설정·토큰지갑 등 org_admin 전용 추가 가이드는 `PAGE_GUIDES`/`TOUR_SCENARIOS` 에 항목을 더하면 된다.
...
- **인터랙티브 가이드(게임 튜토리얼식 둘러보기)** — 정적 안내를 넘어 실제 화면에서 대상 요소를 스포트라이트(딤+구멍) + 글로우 링 + 말풍선으로 짚어 준다. 페이지 진입 시 자동 노출(아래 "페이지 진입 가이드").
  - 정의: `lib/tour-scenarios.ts` (**4 시나리오** = 공고 등록[`member-job-new`, 4스텝] / 공고 페이지[`member-job-page`, 7스텝] / 후보 페이지[`member-candidate-page`, 4스텝] / 법인 설정[`org-settings`, 4스텝, org_admin 전용]). 각 스텝은 `path`(+hash, `{jobId}`/`{candidateId}` 치환) + `target`(CSS 선택자, 페이지의 `data-tour="..."` 속성) + 말풍선 문구 + 선택적 `presets`(이동 전 localStorage 세팅) + 선택적 `popup`(`{open, close?}` 셀렉터 — 드로어/모달/탭을 가이드가 직접 열고 닫으며 따라하게 함).
  - 상태: `app/components/tour/tour-store.ts` — sessionStorage 영속 + 구독 모듈 싱글톤(컨텍스트 대신). 라우트 이동에도 유지, 새로고침 복원. `ActiveTour.dismissKey`(페이지 진입 가이드에만 설정) → 말풍선 마지막 단계 '다시 보지 않기' 체크박스.
  - 오버레이: `app/components/tour/TourOverlay.tsx` (layout 마운트, system_admin 제외). 경로 불일치 시 `router.push` → 대상 요소 폴링(~5s) → 중앙 스크롤 → 스포트라이트(box-shadow 트릭) + 글로우 링(globals.css `tour-glow-ring`) + 말풍선(이전/다음, **키보드 →/Enter/Space=다음 · ←=이전 · Esc=닫기** — 폼 입력·포커스된 버튼/링크에선 가로채지 않아 입력·이중진행 방지). 한 페이지 다단계 가이드를 키보드로 넘기게. 컨테이너 `pointer-events-none` 라 사용자가 안내 중에도 실제로 따라 할 수 있음. 목적지가 접근 거부(PIN·권한·만료·삭제)로 다른 경로로 튕겨내면 무한 재이동 대신 종료(`navAttemptRef` 1회 가드). **팝업 동기화**: 스텝에 `popup={open,close?}` 가 있으면 진입 시 닫혀 있을 때만 `open` 셀렉터를 클릭해 열고(`openPopupRef` 추적), 다음 스텝이 다른 팝업을 원하거나 가이드를 떠나면(`endTour`) `close` 를 클릭해 닫는다. **열림 판정**(`isPopupOpen`)은 open 트리거의 `aria-pressed` 우선(늘 마운트된 채 translate 로 숨는 토론 드로어·토글 대응), 없으면 대상 존재로(언마운트형 드로어·모달). `close` 생략 = **탭처럼 닫을 필요 없음**(다음 단계 open 이 전환). 연속 스텝이 같은 `open` 을 가리키면 그 사이엔 닫지 않음(드로어 안 2스텝 등). 모달 닫기는 백드롭(`[role="dialog"][aria-modal="true"]`) 클릭으로 처리.
  - 대상 요소 마킹: `data-tour="apply-link-new"`·`job-import-url`·`eval-focus`·`job-password`(`/jobs/new`), `resume-intake-btn`·`resume-intake-close`·`apply-link`·`upload-zone`·`interviewers-inline`·`candidate-list`(`/jobs/[id]`), `mcq-btn`·`mcq-body`(`/jobs/[id]` 역량평가 모달, `mcq-panel.tsx`), `cand-tab-{overview|screening|…}`·`cand-overview`·`cand-screening`(`/candidates/[id]` 탭/콘텐츠), `cand-stage-next`(단계 변경 버튼, `stage-panel.tsx`), `cand-discuss-btn`·`cand-discuss-panel`·`cand-discuss-close`(토론 드로어, `candidate-chat.tsx`), `cfg-address`·`#culture-fit`·`cfg-smtp`·`cfg-zoom`(`/org/settings`).
- **페이지 진입 가이드 (법인담당자·면접관 공통)** — 진입한 페이지의 할 일을 안내한다. 순서·데이터 완료 판정이 아니라 "이 사용자가 이 페이지 가이드를 껐는가"(계정별 `users.seen_member_guides`)로만 노출을 정한다.
  - `/jobs/new` 첫 진입 → `member-job-new` 시나리오 **4스텝**(스포트라이트만, 팝업 없음): ① 지원 링크 생성(`apply-link-new` — 저장 전 링크 발급 → 채용사이트 '지원하기'에 연결) ② 기존 공고 URL 자동 채우기(`job-import-url` — 본문·이미지 분석) ③ AI 평가 중점 사항(`eval-focus` — 공고 본문엔 없는 내부 평가 우선순위, HR 전용) ④ 공고 비밀번호(`job-password` — 비지정 멤버 잠금 + '공유'로 면접관 초대 시 자동 등록되어 PIN 우회, `lib/job-lock.ts`). **awaitTarget=`apply-link-new` + `desktopOnly`** — 데스크톱 폼에 항상 렌더돼 진입 시 항상 시작(모바일은 폼 대신 안내만 뜨는 데스크톱 전용 화면).
  - `/jobs/{id}` 첫 진입 → `member-job-page` 시나리오 **7스텝, 4묶음**: ① 이력서 등록(`resume-intake-btn` 소개 → 드로어 열어 `apply-link` → `upload-zone` → 닫기) ② 역량평가(`mcq-btn` 소개 → 모달 열어 `mcq-body` → 닫기) ③ 면접관 지정(`interviewers-inline`) ④ 이력서 목록(`candidate-list`). 드로어·모달은 `popup={open,close}` 로 가이드가 직접 열고 닫는다. **awaitTarget=`resume-intake-btn` + `desktopOnly`** — 후보 1명+ 있고(헤더 '이력서 받기' 버튼 존재) 데스크톱(sm+, 드로어·모달이 sm+ 전용)일 때만 시작. 0명이면 버튼이 없어 시작·기록 안 함(다음 공고에서 재시도).
  - `/candidates/{id}` 첫 진입 → `member-candidate-page` 시나리오 **4스텝**: ① 종합평가(`cand-overview`, 종합평가 탭) ② 서류평가(`cand-screening` — 서류평가 탭으로 전환) → 단계 변경(`cand-stage-next`, "여기서 AI 면접 요청 발송") ③ 토론(`cand-discuss-panel` — 토론 드로어 열어 안내). 탭 전환은 `popup.open`(close 생략), 토론 드로어는 `popup={open,close}`(토글이라 `aria-pressed` 로 열림 판정). **awaitTarget=`cand-stage-next` + `desktopOnly`** — '단계 변경' 버튼이 있는(다음 전형이 있는, 진행 가능한) 후보·데스크톱일 때만 시작(종결 후보엔 안 뜸).
  - `/org/settings` 첫 진입 → `org-settings` 시나리오 **4스텝**(스포트라이트만, 팝업 없음): ① 회사 주소(`cfg-address`, 오프라인 면접 메일에 포함) ② 컬처핏·정성평가(`#culture-fit`, NCS 직업기초능력 기반) ③ 메일 서버(`cfg-smtp`, 자사 도메인 발송·미설정 시 Intervia) ④ 화상 면접(`cfg-zoom`, Zoom 연동 시 일정 확정→방 자동 개설·링크 발송). **awaitTarget=`cfg-address`** — `/org/settings` 는 법인담당자만 접근하므로 자연히 org_admin 전용(`desktopOnly` 미설정 — 단순 스포트라이트라 모바일도 동작).
  - 자동시작·기록: `TourAutoStart`(`PAGE_GUIDES`, system_admin 제외 전 역할 공통 — `layout.tsx`). `awaitTarget` 이 떠야 시작 → 잠긴 공고(언락 전)·후보 0명·모바일·종결 후보(단계 변경 버튼 없음)에선 시작 안 함(정상 진입 시 재시도). **기록 시점**: 노출 즉시가 아니라, 가이드 **마지막 단계의 '다시 보지 않기'를 체크하고 종료**할 때만 `POST /api/orgs/me/member-guides`(키별=페이지별). 끄기 전엔 진입할 때마다 다시 안내(`ActiveTour.dismissKey` → `TourOverlay` 말풍선 체크박스 → `endTour`).
...
- `buildInterviewQuestionsPrompt`: 1차 대면 면접 질문지 (이력서+서류평가+AI면접 평가+법인 컬쳐핏 기준(있으면) 종합 → 섹션별 맞춤 질문지). 1차 일정 확정 후 면접관이 생성, `interview_question_sheets` 에 저장 (round='round1'). 분량 **총 48~64문항(섹션 7~10)** — 면접관이 골라 쓰는 풀. 기술 편중을 막으려 14개 관점 축(경험·성과/이력서 사실 확인/이직 사유·커리어/협업·갈등/문제해결/일하는 방식/학습/조직 적응/실무 확인/컬처핏/상황 대처/동기 등)을 열거하고 기술·직무 섹션은 최대 2개로 제한, **이력서 사실 확인·실무 확인 사항 섹션은 필수**(경력 공백·짧은 재직·담당 범위 실체 / 입사 시점·희망 처우·근무 조건·겸업·레퍼런스).
- `buildExecutiveInterviewQuestionsPrompt`: 2차(임원) 면접 질문지 — 기술 재검증 없이 임원 관점 축(컬쳐핏·인재상 적합성/가치관·태도/조직 기여/성장 잠재력/동기·정착/커리어 흐름 확인/리더십·영향력/위기·변화 대응/윤리·판단/기대치 정렬)에 집중. 분량 **총 24~36문항(섹션 6~8)**, 직무 섹션은 1개·4~6문항까지. **커리어 흐름 확인·기대치 정렬 섹션 필수.**
- 두 질문지 모두 `QUESTION_SHEET_SCHEMA`(prompts.ts)를 `responseSchema` 로 넘긴다 — 분량 2배 이후 자유서술이 길어져 스키마 없이는 간헐적으로 깨진 JSON 이 나온다(실측 3회 중 1회 파싱 실패, 스키마 적용 후 0건). 생성 소요는 1차 약 2~2.5분이라 라우트 `maxDuration=300`(stale 판정 5분과 동일). 법인 컬쳐핏 기준(`organizations.culture_fit_profile`, 있으면)을 질문 설계의 중심 축으로 주입. 2차 일정 확정 후 생성, 같은 테이블에 round='round2' 로 저장. 과금은 1차와 동일 키(`interview_question_gen`) — UI 단가 표기도 "면접 문제 생성" 단일.
