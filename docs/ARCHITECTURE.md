...
- **대시보드 첫 실행 가이드** (`app/page.tsx` `SetupGuide`, **법인담당자 org_admin 전용**): 법인의 "인재상·컬쳐핏 확인 → 공고 → 지원 링크 → 이력서 → AI 면접" 첫 사이클 완료 여부로 5스텝 진행도를 계산. (멤버는 아래 "멤버(면접관) 페이지 가이드"를 쓴다). 스텝 정의·순서·문구는 `lib/setup-steps.ts` `buildSetupSteps` 한 곳에서만 관리하고, hero/strip(`app/page.tsx`)·플로팅 위젯(`SetupGuideWidget`)이 공유하므로 진행도 분모(`steps.length`)는 하드코딩이 아니라 동적이다.
  - 진행 판정: `setup1`=`organizations.culture_fit_profile` 저장됨(설정 페이지 기본값 확인 후 저장 1회, CTA 는 `/org/settings#culture-fit`), `setup2`=공고>0, `applyLink`=공개 지원 링크(`job_postings.apply_token`)를 발급한 공고가 1개 이상, `setup3`=후보>0, `setup4`=stage 가 `ai_pending` 이상 도달(후보자 집계 `interviewReached`). **세 곳이 같은 판정을 각자 SQL 로 재현**하므로(대시보드 `app/page.tsx`, 위젯 API `app/api/orgs/me/setup-progress`, 자동노출 `TourAutoStart`) 조건 변경 시 모두 동기화 필수.
  - **공고 0개** → KPI/알림/공고목록 대신 hero 가이드만 노출 (가장 비어 있는 시점에 가장 빽빽하던 화면을 제거).
  - **일부 진행** → 대시보드 상단에 슬림 진행 스트립(`시작 가이드 N/5` + 다음 단계 CTA). **전 스텝 완료** → 자동 숨김. 스텝 추가(컬처핏·지원 링크)로 기존 단계만 완료한 법인도 신규 스텝 미완이면 스트립이 다시 노출(신기능 확인 유도 — 의도된 동작).
...
- **인터랙티브 가이드(게임 튜토리얼식 둘러보기)** — 정적 안내를 넘어 실제 화면에서 대상 요소를 스포트라이트(딤+구멍) + 글로우 링 + 말풍선으로 짚어 준다. **별도 런처 섹션 없이, 시작 가이드 단계 항목 자체가 곧 둘러보기 버튼**(단계를 누르면 단순 이동 대신 해당 투어 실행).
  - 정의: `lib/tour-scenarios.ts` (5 시나리오 = 컬쳐핏 등록 / 공고 등록 / 지원 링크 / 이력서 업로드[2스텝] / AI 면접). 각 스텝은 `path`(+hash, `{jobId}`/`{candidateId}` 치환) + `target`(CSS 선택자, 페이지의 `data-tour="..."` 속성) + 말풍선 문구 + 선택적 `presets`(이동 전 localStorage 세팅 — 접힌 Section 펼치기, 키도 `{jobId}` 치환). 단계↔시나리오 매핑은 `lib/setup-steps.ts` `SetupStep.tour`. **지원 링크·이력서 업로드는 같은 `/jobs/{id}` 페이지에 있어, `TourAutoStart` 의 org_admin 자동노출은 같은 경로에 걸린 가이드 중 미완료인 첫 시나리오(지원 링크 → 이력서)를 고른다.**
  ...
  - 단계 런처: `app/components/tour/guide-steps.tsx` `GuideStepList`(hero/widget 공용) + `GuideStripCta`(슬림 스트립). 시작 가이드(`app/page.tsx`)와 플로팅 위젯(`SetupGuideWidget`)이 공유. `GET /api/orgs/me/tour-targets`(최신 공고 id + `stage='screened'` 미종결 후보 id)로 활성/잠금 판정 — **지원 링크·이력서 업로드 시나리오는 공고가 있어야, AI 면접 시나리오는 평가 끝난 이력서가 있어야 활성**.
  - 대상 요소 마킹: `data-tour="job-import-url"`(`/jobs/new`), `apply-link`·`consent-gate`·`upload-zone`·`job-header`·`interviewers-inline`·`candidate-list`(`/jobs/[id]`), `screening-report`·`ai-interview-btn`(`/candidates/[id]`), `#culture-fit`(`/org/settings`).
