...
- **잔액 정책 (혼합)** — 공고 생성은 가드 없이 후불로 마이너스까지 진행(선차감이라 잔액이 음수로 떨어질 수 있음). **그러나 잔액 0 이하면 유료 행위는 402 로 하드 차단** (`lib/wallet-guard.ts` `requirePositiveBalance`): 이력서 업로드·서류 평가/재평가·면접 링크 발급(단건/일괄)·면접 시작·결정 통보 메일·일정 제시. system_admin·orgId=null 은 통과. `/org/tokens` 경고 배너 + 0 이하 전환 시 org_admin 메일 알림. → "잔액 0 이하여도 모든 기능 계속 동작"은 **아님**(과거 서술 정정). 충전하면 즉시 재개.
