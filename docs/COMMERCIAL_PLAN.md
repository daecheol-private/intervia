...
> - 연장 비용 = (보관 중 이력서 수) × resume_upload 단가
>   - 보관 중 = outcome 이 NULL(진행 중) 또는 hired(합격). 불합격·지원취소는 파일이 폐기되어 보관비용이 없으므로 과금 제외 (2026-06-05 변경)
...
- 2026-06-07 — **Sentry 에러 페이로드 PII 스크럽**. Sentry 가 미국 리전(`ingest.us.sentry.io`)이라, 에러 전송분에 개인정보가 섞이면 §28의8 국외이전이 됨. `lib/error-reporter.ts` 가 전송 직전 `error.message`·stacktrace·context 의 문자열에서 이메일·전화·주민번호·면접/일정 토큰(tk_/sch_)·UUID·hex 를 마스킹하고, 이름·자유서술성 키(name/email/metadata/text 등)는 통째로 redact. 숫자 식별자(candidateId 등)는 디버깅용 유지(직접 식별 불가). `instrumentation.ts` 는 raw request.path(토큰 박힘) 대신 routePath(패턴)만 전송. Vercel 로그엔 원본 유지(디버깅 무영향) — 스크럽은 Sentry/Slack 전송분 한정. tsc 통과 + 스크럽 정규식 검증. (DSN 은 javascript-nextjs 프로젝트로 교정 — 이전 DSN 이 다른 프로젝트를 가리켜 이벤트 미수신이던 문제 해결.)
...
- 2026-06-18 — **오픈베타 가격 정책 도입** (홍보·콜드스타트용). **AI 면접·대면 면접 평가**를 정가 30토큰(3,000원) → 베타 **10토큰(1,000원)**, 표시상 **~2026년 7월 31일**(변동 가능). 이력서 평가(3토큰/300원)·가입 보너스 500토큰·공고/문제생성 무료는 **유지**.
  ...
  - **⚠️ 표시가 ≠ 실제 차감가 사고 (2026-09-17 수정)**: 2026-07-24 `/admin/pricing` 에서 이력서 평가 3→5 토큰, 면접 문제 생성 0→5 토큰으로 올렸는데 `lib/beta.ts` 코드값을 같이 안 고쳐, 공개 요금표(`/pricing`·랜딩 — 코드값 표시)는 300원·무료인데 실제 차감(`token_pricing` DB override)은 500원·500원이었다. 토스 심사용 결제경로 캡처를 만들다 `/pricing` 과 `/org/tokens` 단가가 어긋난 것으로 발견. **코드값을 실제 차감가(5·5)로 맞춤** + `app/features/guide-data.tsx` 의 "면접 문제 생성 무료" 문구 정정. 재발 방지: `/admin/pricing` 에서 단가를 바꾸면 `lib/beta.ts` 를 같이 고칠 것(주석에 명시).
...
- 2026-06-22 — **토큰 충전 결제 연동 (토스페이먼츠, 카드 단건결제)** — 사업자등록 완료 후 자가결제 도입. 그동안 system_admin 수동 충전(`grant-tokens`)만 가능하던 것을 org_admin 자가결제로 확장. **DB 스키마 변경 없음** — 기존 `payment_orders` 테이블 + `applyChargePayment()`(멱등) 재사용. 토스 `orderId = IV-{payment_orders.id}`(PK 가 곧 전역 유니크 주문번호, 마이그레이션 회피), paymentKey 는 기존 `provider_ref` 컬럼에 저장.
  ...
  - **2026-09-13 — 카드 충전 게이트(`canChargeByCard`, lib/toss.ts)**: 운영 `payment_orders` 에 결제 시도 5건(08-17~09-10, 전부 failed)이 있어 운영에 테스트 키가 들어간 상태로 보임 — 테스트 키가 전 법인에 열려 있으면 가짜 결제로 토큰이 무상 지급되는 구멍. 규칙: 키 없음=불가 / 라이브 키=전 법인 / 테스트 키=로컬은 전 법인, Vercel(운영·프리뷰)은 `TOSS_TEST_ORG_IDS`(쉼표 구분 법인 id = 가맹 심사용 법인)만. checkout·confirm(pending 승인)·`GET /api/orgs/tokens`(`cardChargeEnabled`)→`ChargePanel enabled` 공통 적용, 테스트 결제 매출 알림엔 `[테스트 결제]` 접두. 라이브 키로 교체하면 자동 해제. DB 변경 없음.
  - **2026-09-13 — 충전 옵션 재구성(토스 1회 결제 10만원 한도, 사용자 결정)**: VAT 별도라 공급가 10만원도 결제 11만원 → 한도 초과. `CHARGE_PACKAGES` 에 `method`(결제액 ≤ `CARD_MAX_PAYMENT_KRW`(10만원)이면 card, 아니면 transfer) 추가 — 카드 1만/3만/5만(보너스 0), 계좌이체 10만/30만/50만/100만(보너스 구간 그대로라 `CHARGE_BONUS_TIERS`·`calcTokensForKrw` 불변). checkout 은 `isCardChargeAmount` 로 카드 패키지만 허용(10만원 이상 400). 계좌이체는 `/org/tokens` 에서 금액 선택 → 고객센터 문의(`billing`, 지급 토큰 계산 포함)로 접수 → 운영자 메일·Slack 알림 → 입금 계좌·세금계산서 안내 → `grant-tokens` 수동 충전. 새 API·DB 변경 없음.
  - **2026-09-16 — 계좌이체 충전 자동화(사용자 설계, 위 문의 접수 방식 대체)**: 신청 = `payment_orders(provider=transfer)` pending + 화면·메일 입금 안내(토스뱅크 1002-6424-0903 예금주 아임인, 입금자명 `IV{주문번호}`, 기한 7일 — 자동 취소 없음) + 운영자 Slack → 고객 "입금 완료 · 확인 요청" → Slack [입금확인] 버튼(`/api/slack/interactions`, 서명 검증 + `SLACK_APPROVER_USER_IDS`) → `applyChargePayment`(신청 때 안내한 토큰 그대로, 멱등) + 충전 완료 메일·인앱 알림. 예비 경로: 관리자 결제 내역 화면 [입금확인](step-up). 계좌이체 [환불] = 주문 취소 + 토큰 회수, 금액은 운영자가 고객 계좌로 직접 송금. 세금계산서는 홈택스 수기 발행(관리자 결제 내역에 상호·사업자번호·공급가/세액 표시). DB: `payment_orders` 컬럼 3개 추가(0066, 순수 ADD COLUMN).
  - **2026-09-16 — 결제 통합 목록·세금계산서 체크 + 카드 3/6/9만(사용자 결정)**: `/admin/payments`(관리 메뉴 "결제") — 전 법인 결제를 월(KST) 단위로 합계(결제·공급가·세액·카드/계좌이체)·CSV. 탭: 결제 완료 / 세금계산서(입금확인된 계좌이체, 미발행은 달과 무관하게 전부) / 전체 기록. 계좌이체는 홈택스 발행 후 체크(`tax_invoice_issued_at/by`, 0067 순수 ADD COLUMN), 운영 대시보드에 미발행 건수. 카드는 카드 매출전표가 증빙이라 세금계산서 대상 아님(같은 거래 이중 발급 X — 사용자는 "카드는 자동 세금계산서"로 알고 있었음, 세무사 확인 권장). 카드 충전 옵션 1/3/5만 → **3/6/9만**(결제 33,000/66,000/99,000원, 10만원 한도 안). ⚠️ 토스 심사 결제경로 캡처(상품 선택 화면)는 새 금액으로 다시.
...
- 2026-07-17 — **만료 로그인 세션 파기 + 처리방침 §3 보유기간 행 신설** (사용자 지적: 마이페이지 "활성 세션"에 만료된 세션이 남아 보임). 마이그레이션 없음.
  - **원인**: 만료 행 삭제가 `getCurrentUser` 의 lazy 경로(그 토큰으로 재요청 시)뿐이라, 기기 교체·쿠키 삭제·장기 미접속이면 행이 무기한 잔존. 로그인 세션 청소 cron 은 부재(`expire-interviews` 는 면접 세션용으로 무관). 보안 문제는 아님(만료 토큰은 `getCurrentUser` 가 거부) — 표시 + 파기 의무 문제.
  - **조치 3건**: ① `GET /api/auth/sessions` 에 만료 행 제외 필터, ② `cleanupExpiredSessions()`(lib/auth.ts) 신설 + 일일 cron(`/api/cron/purge-original`, `cleanupOldAttempts`·`cleanupOldRateLog` 와 동일 패턴)에 배선 → 응답에 `purgedSessions`, ③ 처리방침 §3 표에 "로그인 세션 기록(접속 IP·User-Agent) — 세션 만료(마지막 활동 후 6시간) 시 파기" 행 추가.
  - **왜 ③이 필요**: IP·User-Agent 는 §2 수집항목에 이미 명시돼 있는데 §3 보유기간 표엔 없어 법정 기재사항(PIPA §30①2호) 누락 상태였다. 표의 "로그인 시도·Rate limit 30일" 은 `auth_attempts`/rate log 라 별건.
  - **버전**: `PRIVACY_VERSION` 1.5.0→**1.6.0-2026-07-17**, `PRIVACY_EFFECTIVE_DATE` 07-17. 파기 강화라 정보주체에 불리한 변경이 아니므로 **재동의 불요**(SOP §4-1 3번 해당 없음) — 세션 IP/UA 는 회원 데이터라 후보자 면접 동의(`CONSENT_VERSION`) 축과도 무관. 시행일은 §12 7일 공지 기준 07-24 로 잡았다가 **서비스 개시 전(기존 정보주체 부재)이라 즉시 시행으로 조정**(사용자 결정 2026-07-17 — 1.5.0 때와 동일 논리).
  - **함정(실측 확인)**: `sessions.expiresAt` 는 ISO('T' 포함, `createSession`·슬라이딩 갱신 모두)라 cutoff 도 ISO 여야 한다. 옆의 `cleanupOldAttempts` 가 쓰는 `sqliteTimestamp`(공백 구분)를 그대로 가져다 쓰면 `'T'(0x54) > ' '(0x20)` 이라 **모든 행이 cutoff 보다 커서 0건만 지우고 정리가 조용히 죽는다**(에러 없음). 녹취 stuck 오판과 원인은 같은 포맷 혼용이지만 **방향은 반대** — 그쪽은 저장값이 공백·cutoff 가 ISO 라 살아있는 걸 stale 로 오판했고, 이쪽은 저장값이 ISO 라 아무것도 안 지워진다. 격리 DB 픽스처(만료 3·유효 2, ±1초 경계 포함)로 목록 필터·삭제·함정 방향 모두 실측(2026-07-17).
- 2026-09-04 — **약관 §7-2 환불 청구 기한 1년 한정 (토스페이먼츠 가맹 심사 보완)** — 토스 회신: "무기한 환불이 가능한 형태는 심사가 불가합니다. 1년 이내 환불만 가능하도록 환불 정책을 수정해 주시기 바랍니다."
  - **원인**: §7-2 "7일 경과 후: … 미사용 잔액은 **언제든** 환급을 청구할 수 있습니다" 가 기한 없는 환불 약정으로 읽힘. §7 토큰 유효기간은 이미 1년(1.4.0, 같은 심사 보완)이라 **두 조문이 서로 모순**이었다 — 유효기간이 지나면 잔액이 소멸해 환급 대상 자체가 없는데 §7-2 은 언제든 청구 가능하다고 적혀 있었다. 따라서 이번 개정은 축소가 아니라 정합성 정정.
  - **조치(약관)**: §7-2 에 "환불 청구 기한: 결제일로부터 1년 이내" 항 신설, "7일 경과 후" → "7일 경과 후 ~ 결제일로부터 1년 이내", 강제 해지 항도 동일 기한. §7 유효기간 항에 "소멸한 토큰은 환급 청구 불가" 명시. §7 단가 인상·§10 해지·§12 개정의 환급 문구는 §7-2 참조로 기한 귀속. §8 장애 보상은 "토큰을 **환불**" → "토큰을 **복구**"(현금 환급과 혼동 제거, 현금은 §7-2).
  - **노출 3곳 동기화**: `/org/tokens` 결제 화면 하단 고지(유효기간·환불 기한 + 약관 링크 — 전상법 §13 거래조건 표시), `/pricing` 유의사항, `/faq` "충전한 금액을 환불받을 수 있나요?" 항목 신설.
  - **버전**: `TERMS_VERSION` 1.4.0 → **1.5.0-2026-09-04**, `TERMS_EFFECTIVE_DATE` 09-04. 형식상 불리한 변경(§12 30일 공지 대상)이나 **운영 DB 읽기전용 확인 결과 유상 충전 이력 0건**(`payment_orders` 는 failed 4건뿐, charge ledger 0행) → 통지 대상 부재로 게시 즉시 시행(1.4.0 과 동일 논리). 유상 충전이 시작된 뒤 기한을 더 줄이려면 §12 30일 공지 필요.
- 2026-09-14 — **처리방침 1.7.0 — 면접 일정 알림톡(면접관·일정 공유 수신자) 반영, 게시 즉시 시행**. 설계·구현: [ALIMTALK.md](ALIMTALK.md) "면접 일정 알림" 절.
  - **내용**: §1 목적에 면접 일정 확정·취소 알림톡 발송(번호를 등록하고 본인이 확인한 사람에 한함), §2 회원 휴대폰 번호(선택)·일정 공유 수신자(이름·이메일·휴대폰 번호(선택))·알림톡 번호 확인 시각, §3 번호·확인 기록 보유기간(받지 않기·삭제·탈퇴 즉시 / 미확인 번호는 확인 기한 7일 + 30일 내 파기), §5 수탁자 알리고·(주)카카오 추가 — 7월부터 운영 중이던 지원자 알림톡의 수탁 고지 누락분도 이 행으로 함께 닫힘.
  - **버전**: `PRIVACY_VERSION` 1.6.0 → **1.7.0-2026-09-14**, `PRIVACY_EFFECTIVE_DATE` 09-14. 선택 항목 신설이고 번호 주인이 확인 버튼을 눌러야 처리가 시작돼 기존 정보주체에 불리한 변경이 아니므로 **재동의 불요**(SOP §4-1 3번 해당 없음). §12 는 7일 전 공지지만 **게시 즉시 시행**(사용자 결정 2026-09-14).
  - **수집 개시 시점**: 코드는 템플릿 승인 전에 배포하고, 번호 등록은 템플릿 코드가 env 에 들어가 `isStaffAlimtalkEnabled()` 가 켜진 뒤부터 열린다 — 처리방침이 수집 개시보다 먼저 시행된다.
