...
- **심사 보완 이력** (토스 요구 → 약관 개정):
  ...
  - 2026-09-13 — **3차 보완**(약관 외 상품·보증보험·서류): ① 1회 카드 결제액 10만원 이하(옵션 선택형) ② 월 정산한도(200만~1,000만) + 서울보증보험 1:1 가입 → 증권번호 회신 ③ 약관에 "충전된 포인트의 이용기간과 환불가능기간은 결제시점으로부터 1년 이내" 문구 그대로 ④ 홈페이지 환불정책 노출 ⑤ 결제경로 PPT(토스 가이드 8구성, 모든 캡처에 주소창). → 약관 §7-1 신설 + §7-2 환불정책 개편(미사용분만 전액 취소, 사용분은 해지·서비스 종료 시에만 미사용 잔액 환불, `TERMS_VERSION` 1.6.0) + 푸터 '환불정책' 링크 + 심사용 법인 전용 테스트 결제 게이트(`TOSS_TEST_ORG_IDS`). 충전 옵션(사용자 결정): VAT 별도라 공급가 10만원도 결제 11만원 → 카드는 **1만/3만/5만원**(결제 11,000/33,000/55,000원, 보너스 없음), **10만원 이상은 계좌이체**(세금계산서 발행, 보너스 유지 — `/org/tokens` 에서 고객센터 문의로 신청 → 운영자가 입금 안내·수동 충전). 남은 것: 보증보험 · 심사용 계정 · 캡처/PPT.
