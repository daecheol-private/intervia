...
| GET/POST | `/api/cron/interview-reminders` | 🌐 (CRON_SECRET) 또는 👑 | ① 확정 대면 면접(`status='selected'`) D-1(24h 전): **면접관**(`interviewer_reminder_sent_at`) + **후보자**(`candidate_reminder_sent_at`) 에게 각 1회. round1/round2 모두. **면접관 쪽은 주말엔 발송 없이 기록만**(월요일 면접은 당일 digest 커버, 후보자는 주말에도 발송). ② **AI 면접 미응답**(`pending`/`in_progress` + 미만료): 링크 발급 후 24h/48h 경과 시 후보자에게 넛지(`interview_sessions.reminder24_sent_at`/`reminder48_sent_at`, 48h 우선·24h skip). 응답 `{schedule, ai}`. Vercel Cron schedule = `0 * * * *` (시간당) |
| GET/POST | `/api/cron/daily-digest` | 🌐 (CRON_SECRET) 또는 👑 | 면접관(`job_interviewers` 배정 active 계정)별로 본인 배정 공고의 '오늘 할 일'을 요약해 1통씩 발송. 4블록(오늘 확정 면접 / 결정 대기 `round1_waiting`·`round1_passed`·`round2_passed` / 신규 지원·검토 대기 `applied`·`screened`·`ai_evaluated` / **자동 종결** — 신규 창 내 `ai_link_expired`·`schedule_link_expired`, expire cron 개별 메일 대체), 각 항목에 `STAGE_WAITER` 라벨. **변동 중심(2026-07-15)**: 24h 내 변동(`updated_at`) 항목은 NEW 배지+블록 상단 정렬+제목에 신규 건수, "오늘 면접 0 + 신규 0"이면 발송 스킵 — 단 월요일(KST)은 변동 없어도 전체 발송(주간 앵커, 신규 창 72h 로 주말분 커버). **주말(토·일)은 당일 확정 면접 있는 면접관에게만 발송**(`shouldSendDigest`). FYI 성 면접관 알림 메일도 주말·평일 야간(21~08시 KST)엔 스킵(`isQuietHoursKst` — 지원 취소 포함 전부, `urgent` 옵션은 미래 예외용). 할 일 0건이면 미발송, 하루 1통 멱등(`daily_digest_logs`). 운영 메일이라 토큰 차감 없음(LLM 미사용). 응답 `{sent, skipped, recipients}`. Vercel Cron schedule = `0 0 * * *` (매일 KST 09:00 = UTC 00:00) |
