...
| DELETE | `/api/admin/inquiries/[id]` | 🔒 👑 (sysadmin) | 문의 삭제 — **system_admin 전용**. 스팸·무가치 문의 정리용. 고객 문의 내역에서도 사라짐(복구 불가). 감사 로그 `inquiry.delete` |
...
| GET | `/api/orgs/tokens/transfer` | 🏢 (admin) | 계좌이체 입금 계좌(`account`)·입금 기한 일수·법인 사업자등록번호·최근 계좌이체 주문 10건(`orders[]`: `depositorName`(IV{id}), `payKrw`(VAT 포함), `dueAt`, `depositNotifiedAt`, `confirmedAt`) |
| POST | `/api/orgs/tokens/transfer` | 🏢 (admin) | 계좌이체 충전 신청 `{amountKrw}` — 계좌이체 패키지만(`isTransferChargeAmount`, 카드 금액 400). pending `payment_orders(provider=transfer)` 생성 → 응답 후 신청자 입금 안내 메일 + 운영자 Slack. 멤버 403, rate-limit 5회/10분 |
| POST | `/api/orgs/tokens/transfer/[id]/notify` | 🏢 (admin) | 입금 후 "확인 요청" — `deposit_notified_at` 기록 + 운영자 Slack 에 [입금확인] 버튼. 10분 안 재요청은 `sent:false`(알림 없음). 타 법인 404, 이미 충전·취소 409 |
| POST | `/api/slack/interactions` | 🌐 (Slack 서명) | Slack 앱 Interactivity Request URL — [입금확인] 버튼 콜백. `SLACK_SIGNING_SECRET` 서명(5분 이내) 불일치 401, `SLACK_APPROVER_USER_IDS` 에 없는 사용자는 처리 안 함(본인에게만 안내). 처리 = `confirmTransferDeposit`(pending→paid 조건부 + 원장 멱등) → 원본 메시지를 완료로 교체 + 신청자 인앱 알림·충전 완료 메일. proxy CSRF·세션 가드 면제 |
...
| POST | `/api/jobs` | 공고 생성. **`job_post` 토큰 차감**. body `traitProfile?`(Big Five 선호 특성 — high 최대 3개, 초과 400), `sourceUrl?`(원본 공고 주소 — 「가져오기」로 채웠거나 입력칸에 주소만 적어둔 경우 둘 다 기록), `sourceImported?`(true 일 때만 `source_imported_at` 기록. 주소만 적은 경우는 링크만 남고 시각은 null) |
| GET | `/api/jobs/[id]` | 🔑 |
| PUT | `/api/jobs/[id]` | 🔑. `traitProfile` 키가 있을 때만 갱신 (high 최대 3개). `sourceUrl` 도 **키가 있을 때만** 갱신 — 클라이언트는 이번 편집에서 임포트를 새로 했거나, 입력칸 주소가 기록된 값과 다를 때 보낸다. `sourceImported=true` 면 `sourceImportedAt`=now, 아니면 null(주소만 기록) |
...
| 메서드 | 경로 | 권한 | 설명 |
|---|---|---|---|
| GET/POST | `/api/cron/expire-interviews` | 🌐 (CRON_SECRET) 또는 👑 | 만료된 면접 세션 → `expired` 처리. **환불 없음**(면접은 후차감이라 미시작/미평가는 과금된 적 없음 — `lib/expire-sessions.ts` `refundedCount=0`). `pending`(AI 미시작) 만료는 자동 불합격(`ai_link_expired`). **면접관 개별 알림 메일 없음(2026-07-15 폐지)** — 24시간 cron 이라 야간·주말 메일 소스였고, daily digest '자동 종결' 블록이 대체. Vercel Cron schedule = `0 * * * *` (시간당) |
...
| GET/POST | `/api/cron/interview-reminders` | 🌐 (CRON_SECRET) 또는 👑 | ① 확정 대면 면접(`status='selected'`) D-1(24h 전): **후보자에게만** 1회(`candidate_reminder_sent_at`). round1/round2 모두, 주말에도 발송(시간 민감). **면접관 D-1 개별 메일은 2026-08-10 폐지** — 면접자 수만큼 메일이 쌓여, daily digest 의 '내일 면접' 블록이 대체(`interviewer_reminder_sent_at` 은 미사용 컬럼으로 잔존). ② **AI 면접 미응답**(`pending`/`in_progress` + 미만료): 링크 발급 후 24h/48h 경과 시 후보자에게 넛지(`interview_sessions.reminder24_sent_at`/`reminder48_sent_at`, 48h 우선·24h skip). 응답 `{schedule, ai}`. Vercel Cron schedule = `0 * * * *` (시간당) |
| GET/POST | `/api/cron/daily-digest` | 🌐 (CRON_SECRET) 또는 👑 | 면접관(`job_interviewers` 배정 active 계정)별로 본인 배정 공고의 '오늘 할 일'을 요약해 1통씩 발송. 5블록(오늘 확정 면접 / **내일 면접** — 면접관 D-1 개별 메일 대체(2026-08-10) / 결정 대기 `round1_waiting`·`round1_passed`·`round2_passed` / 신규 지원·검토 대기 `applied`·`screened`·`ai_evaluated` / **자동 종결** — 신규 창 내 `ai_link_expired`·`schedule_link_expired`, expire cron 개별 메일 대체), 각 항목에 `STAGE_WAITER` 라벨. **변동 중심(2026-07-15)**: 24h 내 변동(`updated_at`) 항목은 NEW 배지+블록 상단 정렬+제목에 신규 건수, "오늘·내일 면접 0 + 신규 0"이면 발송 스킵 — 단 월요일(KST)은 변동 없어도 전체 발송(주간 앵커, 신규 창 72h 로 주말분 커버). **주말(토·일)은 당일 또는 다음 날 면접이 있는 면접관에게만 발송**(`shouldSendDigest` — 내일치까지 보는 이유는 이 메일이 D-1 을 대체하기 때문). FYI 성 면접관 알림 메일도 주말·평일 야간(21~08시 KST)엔 스킵(`isQuietHoursKst` — 지원 취소 포함 전부, `urgent` 옵션은 미래 예외용). 할 일 0건이면 미발송, 하루 1통 멱등(`daily_digest_logs`). 운영 메일이라 토큰 차감 없음(LLM 미사용). 응답 `{sent, skipped, recipients}`. Vercel Cron schedule = `0 0 * * *` (매일 KST 09:00 = UTC 00:00) |
| GET/POST | `/api/cron/blob-orphans` | 🌐 (CRON_SECRET) 또는 👑 | **Blob 고아 파일 스위퍼** — Blob 전체 목록 vs DB 파일 키 컬럼 전수(resume/photo/attachment/audio/logo) 대조. **30일+ 미참조는 자동 삭제**(48h~30일은 "관찰 중" 집계만), **서킷 브레이커**: 삭제 대상 > max(50, 전체 10%) 면 컬럼 누락 의심으로 삭제 중단+경보. 삭제·실패 건수 Slack 통지(파일명은 인증된 응답에만). ⚠️ 새 파일 키 컬럼 추가 시 `collectKnownKeys` 등록 필수. Vercel Cron schedule = `0 4 1 * *` (매월 1일) |
| GET/POST | `/api/cron/blob-migrate` | 🌐 (CRON_SECRET) 또는 👑 | **legacy public Blob → private 스토어 마이그레이션 (일회성 도구, cron 미등록)**. GET=dry-run 집계, POST `?limit=N` 배치(fetch→put→get 크기검증→DB 키 UPDATE, 원본 미삭제). 2026-07-12 104건 완료·잔여 0 — legacy 재발 시 재사용 가능 |
...
| GET | `/api/admin/payments?month=YYYY-MM&view=paid\|invoice\|all` | 👑 | 전 법인 결제(충전) 통합 목록 — 월(KST) 단위, 기본 이번 달. `paid`(기본) 결제 완료(카드 승인·계좌이체 입금확인)+그 뒤 환불 / `invoice` 세금계산서 대상 = 입금확인된 계좌이체(미발행은 달과 무관하게 전부, 미발행 먼저) / `all` 대기·실패 포함. `totals`: 건수·결제합계·공급가·세액·카드/계좌이체·환불 건수·미발행(`invoicePending` 이번 달 / `invoicePendingAll` 전체). 결제 시점 = 계좌이체 입금확인 시각, 카드 주문 시각 |
| PATCH | `/api/admin/payments/[id]/tax-invoice` | 👑 | 계좌이체 세금계산서 발행 체크 `{issued: boolean}` → `tax_invoice_issued_at/by` 기록·해제(재체크 시 첫 발행 시각 유지). 카드 400(매출전표가 증빙), 입금확인 전 409, 없음 404 |
