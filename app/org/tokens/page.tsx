...
export default function TokensPage() {
  ...
  if (err)
    return (
      <main className="max-w-4xl mx-auto px-6 py-8 text-danger text-sm">
        {err}
      </main>
    );
  if (!data)
      ...
      <section className="mb-6">
        ...
        <div className="mt-3 flex items-center justify-between gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
          <p className="text-xs text-slate-600">
            결제 시스템 연동은 준비 중. 현재는 시스템 관리자가 수동 충전합니다.
          </p>
          <button
            disabled
            className="px-3 py-1.5 text-xs bg-slate-200 text-slate-400 rounded-lg cursor-not-allowed shrink-0"
          >
            결제 (준비중)
          </button>
        </div>
