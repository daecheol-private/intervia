...
const WINDOWS_STEPS: string[] = [
  ...
  "아래 '데스크톱 앱이 마이크에 액세스하도록 허용'에서 사용 중인 브라우저(Chrome·Edge 등)가 켜져 있는지 확인합니다.",
  "브라우저로 돌아와 면접 페이지를 새로고침한 뒤 마이크 버튼을 다시 눌러 보세요.",
];

const MAC_STEPS: string[] = [
  "주소창 왼쪽의 자물쇠(또는 ⓘ) 아이콘을 클릭 → '마이크'를 '허용'으로 바꾸고 페이지를 새로고침하세요. (가장 흔한 원인)",
  "화면 왼쪽 위  메뉴 → '시스템 설정' → '개인정보 보호 및 보안' → '마이크'로 이동합니다.",
  "목록에서 사용 중인 브라우저(Chrome·Safari·Edge 등)의 스위치를 켭니다.",
