/**
 * Intervia 로고 마크 — 네이비 스퀘어클 배지 안에 흰색 "i" + 코랄 슬래시(SVG).
 * i(점+세로 바)는 흰색, 슬래시(/)는 accent 코랄 — 'iv'를 형상화. 배지는 브랜드 네이비 그라데이션.
 * 워드마크는 'Intervia'(점 없음). 헤더·로그인·면접 화면 등 어디서나 동일.
 *
 * 구현: 원본 "i/" 외곽선을 코랄로 깔고, 그 위에 흰색 세로바·점을 덮어 슬래시만 코랄로 노출.
  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden
      >
        {/* 원본 "i/" 외곽선 전체 — 코랄(accent)로 채움. 위에서 흰 세로바·점이 덮여 슬래시만 노출됨.
