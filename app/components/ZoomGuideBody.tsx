...
import { ZoomShareLinkButton } from "@/app/components/ZoomShareLinkButton";

/**
 * 줌(Zoom) 연동 설명서 본문 — 관리자용(/org/zoom/guide)과 계정 없는 담당자용
 * 공개 페이지(/zoom-guide)가 함께 사용한다. 원본 문서: docs/ZOOM_SETUP_GUIDE.md
 * (내용 동기화 유지).
 *
 * 1단계(줌에서 앱 만들기)는 두 대상이 동일하고, 상단 안내와 2단계만 갈린다:
 *  - "admin"  : 로그인한 org_admin. 만든 값 3개를 본인이 바로 Intervia 설정에 입력.
 *  - "public" : Intervia 계정 없이 링크만 받은 담당자. 값 3개를 요청한 관리자에게 전달.
 */
export function ZoomGuideBody({ variant }: { variant: "admin" | "public" }) {
  ...
  return (
    <main className="max-w-6xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="min-w-0">
          ...
          {isPublic ? (
            <p className="text-sm text-ink-muted mt-1 leading-relaxed">
              이 안내는 Intervia 를 사용하는 회사의 요청으로 공유되었습니다. 아래
              1단계를 따라 회사 줌 계정에서 연결 정보 3개를 만든 뒤, 요청한 담당자에게
              그 값을 전달하면 됩니다. Intervia 계정은 필요 없고, 약 10분이면 끝나며
              코딩도 필요 없습니다.
            </p>
          ) : (
            <p className="text-sm text-ink-muted mt-1 leading-relaxed">
      ...
      <div className="space-y-6">
        ...
        {isPublic ? (
          <Section title="2단계 · 만든 값 3개를 담당자에게 전달하기">
            <p className="text-sm text-ink-soft leading-relaxed">
              Intervia 계정이 없어도 괜찮습니다. 1단계 ③에서 복사한 값 3개를 이
              안내를 보내 준 담당자에게 전달하면, 담당자가 Intervia에 입력합니다.
            </p>
            <ul className="list-disc list-inside text-sm text-ink-soft space-y-1 mt-2 leading-relaxed">
              <li>
            ...
            <Warn>
              Client Secret 은 비밀번호와 같습니다. 전달 후에는 메모를 지워도
              되고, 가능하면 담당자와 약속된 안전한 방법으로 전달하세요.
            </Warn>
          </Section>
        ) : (
