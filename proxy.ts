...
export const config = {
  ...
  matcher: [
    // 인증 가드 대상 경로
    "/((?!login|signup|privacy|terms|legal|password-reset|verify|interview|invite|schedule|unsubscribe|preview|apply|api/apply|api/auth|api/interview|api/uploads|api/orgs|api/invites|api/schedule|api/cron|api/internal|api/health|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|avif|ico|woff2?|ttf|otf|eot)$).*)",
