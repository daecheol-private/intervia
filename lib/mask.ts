...
// "그룹(?!웨어)" — "그룹웨어" 는 회사가 아니라 IT 일반명사다. 가드가 없으면 앞 단어까지 묶여
// "엑스퍼넷 Effice 그룹웨어" → "엑스퍼넷 [회사]웨어", "모바일 그룹웨어" → "[회사]웨어" 로
// 프로젝트명이 소실된다(실측: 이력서 한 건에서 6곳). 조사는 막지 않는다 — "LG그룹에서" 는 계속 가린다.
const RE_COMPANY_SUFFIX =
  /[가-힣A-Za-z0-9·\-&]{2,30}\s*(?:주식회사|㈜|\(주\)|부설연구소|연구소|컴퍼니|그룹(?!웨어)|코퍼레이션|Corporation|Corp\.?|Inc\.?|LLC|Ltd\.?|Co\.?\s*,?\s*Ltd\.?)/g;
// 한글 "주식회사" 도 접두어로 — 표 PDF 는 회사명 칸을 "주식회사아이투엘" 처럼 공백 없이 뱉는데,
// RE_COMPANY_SUFFIX 는 "주식회사" 앞에 2글자 이상을 요구해 이 형태를 통째로 놓쳤다(실측 3건).
const RE_COMPANY_PREFIX =
  /(?:㈜|\(주\)|주식회사|株式会社)\s*[가-힣A-Za-z0-9·\-&]{2,30}/g;
...
const LABELS: LabelRule[] = [
  ...
  // 라벨 뒤에 글자가 이어지면 라벨이 아니다(MobileIron·Telegram·전화번호 정규화…), 값에 숫자나
  // 이미 마스킹된 [전화] 토큰이 없으면 전화번호가 아니다. 두 가드가 없으면 "Mobile Device
  // Management", "고객 전화를 처음 받아 처리하며" 같은 업무 설명이 통째로 [전화] 가 되고,
  // 심지어 이미 치환된 "[전화]휴대폰[이메일]Email" 의 "전화" 를 라벨로 재해석해 뒤를 또 삼킨다
  // (실측 93건 중 75건이 오탐, 그중 42건이 이 토큰 재해석). 실제 번호는 RE_PHONE 이 선마스킹한다.
  {
    re: /(연락처|전화|휴대폰|핸드폰|Mobile|Phone|Tel|電\s*話|电\s*话|手\s*[機机])(?![가-힣A-Za-z])\s*[:：·▶▷-]?\s*(?=[^\n,]{0,30}?(?:\d|\[전화\]))([^\n,]{6,30})/g,
    token: "[전화]",
  },
  {
  ...
  // 회사 라벨은 두 갈래다. 산문에 나올 일 없는 **명시 라벨**("소속회사"·"회사명")은 구분자 없이도
  // 값을 가린다 — 표 PDF 가 "회사명유니에스아이엔씨업종…" 처럼 라벨과 값을 붙여 뱉기 때문이다.
  // 반면 "회사/직장/소속" 같은 **모호 라벨**은 구분자를 요구한다. 느슨하게 두면 자기소개서가 통째로
  // 사라진다 — "회사의 목표와 방향성을 이해하고", "직장인으로 당연한 말이지만", "회사내규에 따름"
  // (희망연봉 칸)이 전부 [회사] 로 치환됐다(실측 93건에서 이런 산문이 138건).
  // lib/pii-extract.ts 의 회사 라벨도 구분자 필수라 추출↔마스킹 범위가 어긋나지 않는다.
  // ⚠️ 명시 라벨 규칙이 반드시 먼저 와야 한다 — 뒤에 두면 "소속회사: 벨로크" 를 모호 라벨의
  //    "소속" 이 먼저 먹어 값 판정이 달라진다.
  {
    re: /(소속회사|재직회사|회사명|직장명|근\s*무\s*처|Company|Employer)\s*[:：·▶▷-]?\s*([^\n,/]{2,40})/g,
    token: "[회사]",
  },
  {
    re: /(회\s*사|직\s*장|소\s*속)\s*[:：·▶▷-]\s*([^\n,/]{2,40})/g,
    token: "[회사]",
  },
  // 라벨 뒤에 글자가 이어지면 라벨이 아니다("부서와의 협업", "부서장 관련 요청", "부 서비스 연동").
  // 예외는 표 셀이 붙어 값이 바로 이어지는 "부서개발1팀직급사원" — 부서 접미사가 뒤따르면 인정한다
  // (RE_DEPARTMENT 는 뒤에 "직급" 이 붙어 있어 이 형태를 못 잡는다).
  {
    re: /(부\s*서|부서명|팀\s*명|소속팀|Department|Dept\.?|Division)(?:(?![가-힣A-Za-z])|(?=[가-힣A-Za-z0-9]{1,15}(?:팀|센터|사업부|그룹|랩|Lab|Team)))\s*[:：·▶▷-]?\s*([^\n,/]{2,40})/g,
    token: "[부서]",
  },
  {
    re: /(직\s*책|직\s*위|직\s*급|Position|Title|Job\s*Title|Role)(?![가-힣A-Za-z])\s*[:：·▶▷-]?\s*([^\n,/]{2,40})/g,
    token: "[직책]",
  },
  ...
  {
    // 한자 男/女 포함 — 요약형 이력서는 "성 별 男" 으로 쓴다. 종전에는 이 값이 성별 라벨에 없어
    // 앞의 전화 라벨이 우연히 삼켜 가려지고 있었다(전화 라벨을 조이면 그대로 노출된다).
    re: /(성\s*별|Gender)\s*[:：·▶▷-]?\s*(남성|여성|남자|여자|[Mm]ale|[Ff]emale|남|여|男|女|M|F)(?![가-힣A-Za-z])/g,
    token: "[성별]",
  },
  {
