/**
 * 불합격 통보 저녁 드레인 — 발신 서버 일일 발송 한도·평판 보호 장치.
 *
 ...
 *
 * 예산 = MAIL_DAILY_BUDGET − 오늘 이미 나간 전체 발송(sentToday) − MAIL_DAILY_BUFFER(20).
 * 버퍼는 드레인 후 저녁~심야에 나갈 수 있는 우선메일(초대·일정 등) 몫을 남긴다.
 * 두 값 모두 운영 환경변수로 주입한다 — 발신 서버 한도가 넉넉하면 예산을 올려 두면 되고,
 * 그때는 낮 소프트캡(REJECTION_DAYTIME_SOFT_CAP)에 걸리는 일 자체가 드물어진다.
 *
