...
export const PRIVACY_VERSION = "1.7.0-2026-09-14"; // 면접 일정 알림톡(면접관·일정 공유 수신자) 반영 — §1 목적에 면접 일정 확정·취소 알림톡 발송(번호를 등록하고 본인이 확인한 사람에 한함), §2 회원 휴대폰 번호(선택)·일정 공유 수신자(이름·이메일·휴대폰 번호(선택))·알림톡 번호 확인 시각, §3 번호·확인 기록 보유기간, §5 수탁자 알리고·(주)카카오 추가(7월부터 운영 중이던 지원자 알림톡의 수탁 고지 누락분 포함). 선택 항목 신설이고 번호 주인이 확인 버튼을 눌러야 처리가 시작돼 기존 정보주체에 불리한 변경이 아니므로 재동의 불요. 시행일: §12 7일 공지 대신 게시 즉시 시행(사용자 결정 2026-09-14). (이전: 1.6.0 — §3 보유기간 표에 로그인 세션 기록(접속 IP·User-Agent) 행 신설 — §2 수집항목엔 있었으나 보유기간이 표에 없어 법정 기재사항(PIPA §30①2호) 누락 상태였다. 실제 파기도 함께 이행(cleanupExpiredSessions 를 일일 cron 에 배선 — 기존엔 해당 토큰 재요청 시에만 lazy 삭제라 무기한 잔존). 파기 강화라 정보주체에 불리한 변경이 아니어서 재동의 불요. 시행일: §12 7일 공지 기준 07-24 예정이었으나 서비스 개시 전(기존 정보주체 부재)이라 게시 즉시 시행으로 조정(사용자 결정 2026-07-17 — 1.5.0 과 동일 논리). (이전: 1.5.0 — AI 처리 장애 폴백 국외이전 고지 신설)
export const TERMS_VERSION = "1.6.0-2026-09-14"; // 토스페이먼츠 3차 보완(2026-09-13 회신) — §7-1 신설: 결제수단 환불·회원(법인) 간 양도 불가·"충전된 토큰(포인트)의 이용기간과 환불가능기간은 결제시점으로부터 1년 이내" 문구를 토스 요구 그대로 기재. §7 카드 1회 결제금액 10만원(부가세 포함) 이하·초과 금액은 계좌이체(세금계산서) 조항 추가. §7-2 환불정책 개편(사용자 결정): 사용하지 않은 충전분(잔액 ≥ 해당 결제 지급 토큰)만 전액 취소, 일부라도 사용했으면 환불 불가 — 해지(탈퇴·강제해지)·회사 서비스 종료 시에만 1년 이내 결제분 미사용 잔액 환불(법인 해지 시 수수료 10% 이내, 회사 종료 시 무공제). 기존 "결제 후 7일 이내 전액(사용분 포함)"·"7일 후 미사용 잔액 상시 환급" 폐지, §7 단가 인상 거부도 해지 후 환불로 정렬. 형식상 불리한 변경(§12 30일 공지 대상)이나 운영 DB 읽기전용 확인 결과 유상 충전 이력 0건(payment_orders: failed 5건뿐, charge ledger 0) → 통지 대상 부재로 게시 즉시 시행. (이전: 1.5.0 —§7-2 환불 청구 기한을 "언제든"(무기한) → 결제일로부터 1년 이내로 한정 — 토스페이먼츠 가맹 심사 요건(2026-09-04 회신: "무기한 환불이 가능한 형태는 심사 불가"). §7 토큰 유효기간이 이미 1년이라 무기한 환급 조항은 그와 모순이었고, 유효기간이 지나면 잔액이 소멸해 환급 대상 자체가 없다 — 실질 불이익 없는 정합성 정정. §8 장애 보상은 "토큰 환불" → "토큰 복구"로 표현 정정(현금 환급은 §7-2로 귀속), §7 단가 인상·§10 해지·§12 개정 조항의 환급도 §7-2 참조로 기한 귀속. 형식상 불리한 변경(§12 30일 공지 대상)이나 운영 DB 확인 결과 유상 충전 이력 0건(payment_orders: failed 4건뿐, charge ledger 0) → 통지 대상 부재로 게시 즉시 시행(1.4.0 과 동일 논리). 유상 충전이 시작된 뒤 환불 기한을 더 줄이려면 §12 30일 공지 필요. (이전: 1.4.0 — 제7조 토큰 유효기간 5년 → 1년 — 토스페이먼츠 가맹 심사 요건(서비스 제공기간 1년 초과 시 입점 불가, 2026-08-18 담당자 회신). 형식상 불리한 변경(§12 30일 공지 대상)이나 카드결제 dormant 로 유상 충전 이력이 없어 통지 대상 부재 → 게시 즉시 시행(1.6.0 개인정보처리방침과 동일 논리). 유상 충전이 시작된 뒤 유효기간을 더 줄이려면 §12 30일 공지 필요. (이전: 1.3.0 — 토큰 유효기간·양도/환금 불가 조항 신설)
