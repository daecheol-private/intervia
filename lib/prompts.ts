...
- 이력서에서 **기간이 있는 이력 항목**을 뽑아 timeline 에 넣는다. 각 항목은 타임라인의 **한 줄**이 된다. kind 정의:
  · **career** = 재직/근무 경력
  · **education** = 학력 — **대학교(전문대 포함)·대학원 이상만. 고등학교·검정고시는 절대 넣지 말 것.**
  · **activity** = 인턴·대외활동·동아리·봉사·수상
...
- start/end 는 "YYYY.MM" 또는 "YYYY". 진행 중이면 ongoing=true + end=null. 시작 시점을 못 찾으면 그 항목은 **넣지 마라**(타임라인 정렬 기준이 없음) — 단, 자격증처럼 취득 시점만 있으면 start 에 그 시점, end=null.
