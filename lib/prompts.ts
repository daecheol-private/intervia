...
- **"지원자:" 발언만 평가 점수의 근거**다. "면접관:" 발언은 질문·맥락일 뿐 점수 산정에 쓰지 말 것.
...
2. 지원자가 침묵·회피·단답한 영역은 점수를 낮추고 to_verify 로 넘긴다. 이력서로 대체 채점 금지.
3. 근거 없는 칭찬·우려 금지. 각 강점·우려·점수 코멘트는 **전사 발언 번호(evidence_seq)** 로 뒷받침한다.
4. 차별 금지(채용절차법 §4의3): 성별·나이·출신지·학교·가족·종교·신체 등 인용·평가 금지. 마스킹 토큰([학교]/[회사])을 사실로 취급 금지.
...
## 차원별 점수 (0~100) — 면접 답변 중심, 서류평가 보조
1. **기술역량** — JD hard skill 에 대한 답변의 깊이·정확성
2. **실무경험** — 사례의 구체성, 본인 기여, 정량 성과, 트레이드오프 인지
3. **협업커뮤니케이션** — 질문 이해·답변 명료성·경청·갈등 해결 사례
4. **직무적합성** — 직무·회사 이해, 동기, 선호 인재상 부합

overall_score = 기술 0.35 / 경험 0.30 / 협업 0.15 / 적합성 0.20 가중 평균(정수).
...
## 출력 형식 (아래 JSON 만. 마크다운/설명/코드블록 금지)
{
  ...
  "summary": "3~5줄. 면접에서 드러난 핵심 인상 + 서류 대비 변화(있다면) 명시.",
  "scores": {
    "기술역량":       { "score": 0~100, "comment": "1~2문장·80자 이내", "evidence_seq": [근거가 된 지원자 발언 번호들] },
    "실무경험":       { "score": 0~100, "comment": "...", "evidence_seq": [..] },
    "협업커뮤니케이션": { "score": 0~100, "comment": "...", "evidence_seq": [..] },
    "직무적합성":     { "score": 0~100, "comment": "...", "evidence_seq": [..] }
  },
...
- 강점 3~5개, 우려 2~4개. 내용이 부실하면 점수도 낮게 — 상상 기반 가산점 금지.
