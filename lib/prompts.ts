  ...
  "timeline": [
    {
      ...
      "start": "시작 시점 \"YYYY.MM\" 또는 \"YYYY\". 이력서에 없으면 null",
      "end": "종료 시점 \"YYYY.MM\" 또는 \"YYYY\". 진행 중이면 null. 없으면 null",
      "ongoing": 현재 재직/재학/진행 중이면 true, 아니면 false,
      "title": "역할·직위·학위·과정명 (기관 '이름' 없이). 예: \\"백엔드 개발 · 팀 리드\\", \\"학사 · 컴퓨터공학\\", \\"AI 부트캠프 수료\\"",
      "highlights": ["그 기간에 **실제로 한 일·성과** (행위 중심 짧은 구절)"]
    }
  ],
  "parsed_check": {
    "education_level": "이력서에 적힌 **최종(가장 높은)** 학력 — 고졸/전문학사/학사/석사/박사 중 하나. 수료·휴학·중퇴면 뒤에 붙여라(예: \\"박사 수료\\"). 불명확하면 null",
    "education_major": "그 최종학력의 전공/학과명 그대로. 여러 개면 하나만. 불명확하면 null"
  }
}

## timeline 규칙 (이력 타임라인 — 평가 점수와 무관, 세로 타임라인으로 표시)
- 이력서에서 **기간 또는 시점이 있는 이력 항목**을 뽑아 timeline 에 넣는다. 각 항목은 타임라인의 **한 줄**이 된다. kind 정의:
  · **career** = 재직/근무 경력
  · **education** = 학력 — **대학교(전문대 포함)·대학원 이상만. 고등학교·검정고시는 절대 넣지 말 것.**
  · **activity** = 인턴·대외활동·동아리·봉사·수상 — **인턴·현장실습은 교육기관이 주선했거나 교육과정 중에 했더라도 activity**(training 아님)
  · **training** = 교육과정·부트캠프·수료·연수·자격증 취득
- **경력(career)은 프로젝트/업무 단위로 잘게 나눈다** — 한 회사 안에서 **각자 기간이 구분되는 프로젝트·업무가 있으면 프로젝트마다 별도 항목**(타임라인 한 줄)으로 만들고, 각 프로젝트의 start/end 를 그 프로젝트 기간으로 넣어라. 프로젝트 구분이 없으면 회사(재직) 단위로 한 항목. **경력이 가장 중요하므로 title 과 highlights 를 가장 상세히** 채운다(어떤 역할로 무슨 일을 했는지).
- **기관 이름은 이미 [회사]/[학교] 로 마스킹돼 있다. title·highlights 에 그 토큰이나 회사/학교 이름을 절대 넣지 말 것.** 오직 "무슨 일을 했는가 / 무엇을 배웠는가"에 집중한다.
- **title** = 프로젝트명·역할·직위·학위·과정명 (예: "SOAR 시스템 구축", "백엔드 개발 · 팀 리드", "석사 · 컴퓨터공학").
- **highlights = 행위·성과 중심 짧은 구절** (예: "결제 시스템 신규 설계", "**일 트래픽 3배** 처리", "React 마이그레이션 리드"). 형용사 나열·일반론·근거 없는 칭찬 금지.
  · **career 는 2~4개로 상세히**, education/activity/training 은 0~2개로 간결히.
- start/end 는 "YYYY.MM" 또는 "YYYY". 진행 중이면 ongoing=true + end=null.
- **자격증·수상·어학은 시점 하나만 있어도 반드시 넣는다** — start 에 그 시점, end=null, ongoing=false. 자격증·어학은 training, 수상은 activity.
  · **"자격증" 섹션의 항목은 하나도 빠뜨리지 마라.** PDF 표가 깨져 발급기관·취득일·자격증명이 **여러 줄로 흩어져** 들어온다. 예를 들어 \`자격증\` 아래에 \`한국산업인력공단2025. 06\` / 다음 줄 \`정보처리기사\` 가 오면 이 둘은 **한 항목**이다 (title="정보처리기사", start="2025.06"). 기관명에 날짜가 붙어 있어도 그게 취득 시점이고, 날짜 줄과 이름 줄의 순서는 뒤바뀔 수 있다.
  · **개수를 맞춰라** — 그 섹션에 \`기관명+날짜\` 줄이 3개면 자격증 항목도 3개다. 오래된 것·급수가 낮은 것도 똑같이 넣는다.
  · 같은 이유로 수상도 \`수상\` 섹션의 기관명·상명·연도가 여러 줄에 흩어진다 — 인접한 줄을 묶어 한 항목으로 만든다.
- **학부·교육과정에서 수행한 프로젝트도 수행 기간이 적혀 있으면 각각 개별 항목**(activity)으로 넣는다. 실무 경력이 없는 후보에게는 이게 사실상 경력 근거이므로, 상위 과정(학위·부트캠프) 항목의 highlights 로 뭉뚱그려 없애지 말 것.
- **날짜를 추정하지 마라.** 이력서에 적힌 시점만 쓴다 — 학년·재학 기간에서 "3학년이니 2021년쯤" 처럼 역산하는 것은 금지다. 시작 시점을 못 찾으면 그 항목은 타임라인에 **넣지 말고**(정렬 기준이 없음), 내용이 중요하면 상위 항목(재직·학력)의 highlights 로 넣어라.
- 이력서에 **실제로 적힌 항목만**. 기간·내용을 지어내지 말 것. 근거 없으면 배열에서 뺀다. 해당 없으면 timeline: [].
- §4의3 제외 항목(출신 학교명·지역·신체·가족 등)은 timeline 에서도 금지 (전공·학위·직무 역할은 무방).

## 형식 규칙
- parsed_check 는 **평가와 무관한 파싱 검증용**이다. 이력서에 적힌 그대로 읽어서 넣되,
  없으면 억지로 만들지 말고 null. 이 값은 점수·판정에 어떤 영향도 주지 않는다.
...
## 강조 표기 (UI 가독성)
- summary / strengths / concerns / interview_focus / timeline.highlights 안에서 **핵심 사실**은 \`**...**\` (markdown bold) 로 감싸라. 각 줄에 보통 1~2개.
...
- **후보자 이력서·사전평가는 개인정보가 마스킹되어 있다. \`[이름]\`·\`[전화]\`·\`[이메일]\`·\`[학교]\`·\`[지역]\` 같은 대괄호 토큰은 마스킹 표시이지 실제 값이 아니다.** 이 마스킹 토큰을 **절대 화면에 출력하거나 후보자 호칭으로 사용하지 말 것.** 후보자의 실명을 모르므로 이름을 지어내거나 추측하지 말고, 호칭은 항상 이름 없이 **"후보자님"** 으로만 한다. (예: "안녕하세요, \`[이름]\` 후보자님" ❌ → "안녕하세요, 후보자님" ✅)
...
## 후보자 이력서 (개인정보 마스킹됨 — \`[이름]\`/\`[전화]\`/\`[이메일]\`/\`[학교]\`/\`[지역]\` 등은 마스킹 토큰이며 실제 값이 아님. 그대로 출력하지 말 것)
${resume}
...
/**
 * 면접 질문지 구조화 출력 스키마 — 아래 두 프롬프트의 "출력 형식" JSON 과 일치.
 *
 * responseMimeType=json 만으로는 긴 한국어 자유서술이 많아질수록 모델이 간헐적으로 깨진
 * JSON 을 뱉어 파싱 실패한다(screening 과 같은 실패 모드). 질문지 분량을 2배(1차 48~64문항)로
 * 늘린 뒤 실측에서 3회 중 1회 파싱 실패가 재현돼 스키마를 강제한다.
 *
 * ⚠️ `Type` enum(@google/genai) 대신 문자열 리터럴을 쓰는 이유: 이 파일은 클라이언트
 * 컴포넌트(app/org/settings)도 import 한다 — SDK 를 import 하면 클라 번들에 딸려간다.
 * 값은 Type.OBJECT 등과 동일한 대문자 문자열이라 Vertex 가 그대로 받는다.
 */
export const QUESTION_SHEET_SCHEMA = {
  type: "OBJECT",
  properties: {
        ...
        properties: {
          ...
          questions: {
            ...
            items: {
              ...
              propertyOrdering: ["question", "intent", "followups", "basis"],
            },
          },
        },
        required: ["title", "focus", "questions"],
        propertyOrdering: ["title", "focus", "questions"],
      },
    },
    red_flags: { type: "ARRAY", items: { type: "STRING" } },
  },
  required: ["strategy", "sections"],
  propertyOrdering: ["strategy", "sections", "red_flags"],
};
 ...
 *
 * 분량은 총 48~64개(섹션 7~10) — 면접관이 골라 쓰는 풀이라 넉넉하게 뽑는다.
 * 기술 검증에 몰리지 않도록 14개 관점 축을 프롬프트에 열거하고, 그중 이력서
 * 사실 확인(경력 공백·짧은 재직·담당 범위 실체)과 실무 확인 사항(입사 시점·
 * 희망 처우·근무 조건·겸업·레퍼런스)은 필수 섹션으로 강제한다 — 회사가 채용
 * 결정 전에 반드시 확인해야 하는 항목이라서.
 */
...
- **기술 검증에 치우치지 말 것.** 기술·직무 역량은 전체의 30~40% 정도이고, 나머지는 아래 다른 관점 축으로 채운다.
- 각 질문에는 면접관이 무엇을 보려는지(intent)와, 답변에 따라 더 캘 꼬리질문(followups)을 붙인다.
- 질문은 면접관이 **그대로 읽을 수 있는 완결된 문장**이어야 한다. **question / followups 안에 대괄호 표기를 절대 쓰지 말 것** — 이력서의 \`[회사]\`·\`[학교]\` 는 개인정보 마스킹 토큰이지 채울 빈칸이 아니다. \`[회사]에서\` → "직전 회사에서", \`[근무지]로 출근\` → "제시된 근무 조건으로 근무". 이력서에 마스킹된 회사가 여럿이라 구분이 필요하면 \`[회사 B]\` 같은 기호를 만들지 말고 **재직 기간으로 지칭**하라("2019년부터 2020년까지 재직하신 회사에서"). JD 에 없는 정보(근무지·연봉·복지 등)는 빈칸을 만들지 말고 빈칸 없이 물을 수 있는 표현으로 바꿔라.
- 차별 금지(채용절차법 §4의3): 성별·나이·출신지·학교·가족·종교·신체·혼인·출산 계획·가족의 직업·재산 등을 묻거나 평가하는 질문 금지. 거주지·통근 거리를 직접 묻지 말고 **"제시된 근무지·근무형태로 근무 가능한지"** 형태로만 확인하라. 마스킹 토큰([학교]/[회사] 등)을 사실로 취급 금지.

## 질문 관점 축 (이 후보자에게 해당하는 축을 골라 섹션으로 구성 — 기술 한 축에 몰지 말 것)
1. **직무·기술 역량 검증** — JD 요구 역량 중 서류로 확인 안 된 부분의 실제 깊이.
2. **경험·성과 심층(STAR)** — 이력서에 적힌 성과가 본인 기여인지, 역할·범위·정량 결과·트레이드오프까지.
3. **이력서 사실 확인** — 회사가 채용 전에 반드시 확인해야 할 것들: 경력 기간·공백, 재직 기간이 짧은 구간의 사유, 적힌 직책·담당 범위의 실체(인원·예산·권한), 프로젝트에서 본인이 실제 맡은 부분, 이력서 표현과 실제 업무의 간극, 자격·면허·필수 요건 보유 여부.
4. **이직 사유·커리어 일관성** — 이직·직무 전환 사유가 일관되는지, 앞으로의 방향과 이 직무가 맞는지, 재직 중 후보라면 이번 이직으로 무엇을 바꾸려는지.
5. **협업·커뮤니케이션·갈등** — 조직 안에서 맡던 역할, 이해관계 조정, 피드백 주고받기, 갈등 사례와 결말.
6. **문제해결·의사결정** — 판단 기준, 정보 부족·시간 부족 상황의 선택, 실패·장애 복기와 재발 방지.
7. **일하는 방식·자기관리** — 우선순위 정하기, 마감·품질 관리, 보고·기록 습관, 주도적으로 벌인 개선.
8. **학습·성장** — 최근 스스로 학습한 것, 배움을 업무에 옮긴 사례, 새 환경·낯선 도메인 적응 경험.
9. **조직 적응** — 이전 조직과 우리 조직의 규모·프로세스·의사결정 속도 차이를 어떻게 받아들일지, 새 팀에 합류할 때의 방식.
10. **실무 확인 사항(회사 관점 체크)** — 입사 가능 시점, 희망 처우 수준과 근거, 제시된 근무형태·근무지 수용 가능 여부, 겸업·경업금지·이전 회사와의 계약 이슈, 레퍼런스 체크 가능 여부, 다른 회사 전형 진행 상황.
11. **인성·컬처핏** — 법인 컬처핏 기준(있으면)의 정성 항목(자기소개서·지원동기·대인관계 등)을 반영한 구체적 질문. 기준의 각 항목이 최소 1회 검증되게 하라.
12. **상황·케이스** — 이 직무에서 실제로 벌어질 가상 시나리오를 주고 판단·대처를 본다.
13. **서류·AI면접 우려 검증** — 앞 단계에서 남은 우려를 직접 확인하는 질문.
14. **동기·기대치 정렬** — 왜 우리 회사·이 직무인지, 입사 후 기대와 현실의 간극, 후보자가 회사에 묻고 싶은 것.
...
## 출력 형식 (아래 JSON 만. 마크다운/설명/코드블록 금지)
{
  ...
  "sections": [
    {
      "title": "섹션 제목 (위 관점 축에서 — 예: 기술 역량 심층 검증 / 경험·성과 / 이력서 사실 확인 / 이직 사유·커리어 / 협업·갈등 / 문제해결·의사결정 / 학습·성장 / 조직 적응 / 실무 확인 사항 / 컬처핏 / 상황 대처)",
  ...
  "red_flags": ["대면에서 반드시 확인해야 할 우려 신호 3~6개 — 이력서·서류평가에서 발견된 확인 필요 항목(경력 공백, 짧은 재직, 담당 범위 불명확, 성과 근거 부족, 요건 미충족 등) 우선 (선택)"]
}

## 분량 (가이드가 아니라 **요구사항**이다)
- 섹션 **8~10개**, 섹션당 질문 **6~8개**. **총 48개 이상 64개 이하.**
- **총합이 48개 미만이면 실패한 출력이다.** 다 쓴 뒤 직접 세어 보고, 모자라면 아직 안 쓴 관점 축을 더 꺼내 섹션을 늘려라. 면접관은 여기서 골라 쓰므로 넉넉해야 한다.
- **위 관점 축 중 최소 8개 축을 섹션으로 다뤄라.** 기술·직무 섹션은 2개를 넘기지 말 것 — 나머지는 경험 검증·이력서 사실 확인·이직 사유·협업·문제해결·일하는 방식·학습·조직 적응·실무 확인·컬처핏·상황 대처로 채운다.
- **"이력서 사실 확인"과 "실무 확인 사항" 섹션은 반드시 포함하라** — 회사가 채용 결정 전에 확인해야 할 것들이다.
- **질문 수가 많아져도 추상적·중복 질문으로 채우지 말 것.** 모든 질문은 이력서·평가 근거에 기반한 **서로 다른 검증 포인트**여야 한다. 같은 내용을 표현만 바꿔 반복 금지 — 근거가 얕은 항목은 상황·케이스 질문으로 바꿔서라도 검증 포인트를 다르게 만든다.
- followups 는 핵심 질문에만 (전부 달 필요 없음). basis 는 가능한 한 채운다.

## 강조 표기 (UI 가독성)
- strategy / focus / question / intent 안에서 핵심 키워드는 \`**...**\` (markdown bold) 로 감싸라. 각 항목 1~2개.

## 출력 직전 자가 점검 (하나라도 어긋나면 고쳐서 다시 써라)
1. 섹션 8개 이상인가? 2. 총 질문 48개 이상인가? 3. "이력서 사실 확인"·"실무 확인 사항" 섹션이 있는가?
4. 기술·직무 섹션이 2개 이하인가? 5. question/followups 에 대괄호 빈칸이 하나도 없는가?`;
}
 ...
 *
 * 분량은 총 24~36개(섹션 5~8) — 1차보다 적지만 임원이 골라 쓸 만큼은 확보.
 * 컬쳐핏 외에 리더십·위기 대응·윤리 판단 축을 더하고, 커리어 흐름 확인(짧은
 * 재직·공백·직무 전환 사유)과 기대치 정렬(희망 처우·입사 시점·근무 조건)은
 * 필수 섹션 — 최종 결정 전에 임원이 직접 확인해야 하는 항목이라서.
 */
...
- 임원 면접의 중심 축 (질문 대부분을 여기서 설계 — 해당하는 축을 골라 섹션으로 구성):
  ...
  · **커리어 흐름 확인(회사 관점)** — 이력서의 경력 흐름에서 임원이 직접 확인할 것: 재직 기간이 짧은 구간·경력 공백의 배경, 직무·업종 전환의 이유, 직책·책임 범위의 실체, 커리어 선택에 일관된 기준이 있는지. **기술 재검증이 아니라 "이 사람의 선택을 신뢰할 수 있는가" 관점.**
  · **리더십·영향력** — 후배·동료를 키우거나 끌어온 사례, 권한 없이 사람을 움직인 경험, 맡길 수 있는 역할의 크기.
  · **위기·압박·변화 대응** — 실적 부진·조직 변화·갈등 국면에서 어떻게 버티고 무엇을 선택했는지.
  · **윤리·컴플라이언스 판단** — 성과와 원칙이 충돌한 상황, 부적절한 요구를 받았을 때의 처신.
  · **기대치 정렬(회사 관점 체크)** — 희망 처우·직책 기대 수준과 근거, 입사 가능 시점, 제시된 근무형태·근무지 수용 가능 여부, 다른 전형 진행 상황, 3~5년 뒤 기대하는 역할. 임원이 조율해야 하는 조건을 미리 확인한다.
...
- 질문은 임원이 **그대로 읽을 수 있는 완결된 문장**이어야 한다. **question / followups 안에 대괄호 표기를 절대 쓰지 말 것** — 이력서의 \`[회사]\`·\`[학교]\` 는 개인정보 마스킹 토큰이지 채울 빈칸이 아니다. \`[회사]에서\` → "직전 회사에서", \`[근무지]\` → "제시된 근무 조건". 마스킹된 회사가 여럿이라 구분이 필요하면 \`[회사 B]\` 같은 기호 대신 **재직 기간으로 지칭**하라("2019년부터 2020년까지 재직하신 회사에서"). JD 에 없는 정보(근무지·연봉·직책 등)는 빈칸 없이 물을 수 있는 표현으로 바꿔라.
- 차별 금지(채용절차법 §4의3): 성별·나이·출신지·학교·가족·종교·신체·혼인·출산 계획·가족의 직업·재산 등을 묻거나 평가하는 질문 금지. 거주지·통근 거리를 직접 묻지 말고 **"제시된 근무지·근무형태로 근무 가능한지"** 형태로만 확인하라. 마스킹 토큰([학교]/[회사] 등)을 사실로 취급 금지.
...
## 출력 형식 (아래 JSON 만. 마크다운/설명/코드블록 금지)
{
  ...
  "sections": [
    {
      "title": "섹션 제목 (위 중심 축에서 — 예: 컬쳐핏·인재상 / 가치관·태도 / 조직 기여·협업 / 성장 잠재력 / 동기·정착 / 커리어 흐름 확인 / 리더십·영향력 / 위기·변화 대응 / 윤리·판단 / 기대치 정렬 / 직무 이해·비전)",
      "focus": "이 섹션으로 확인하려는 핵심 (한 줄)",
      "questions": [
        {
          "question": "후보자 맞춤 구체 질문",
  ...
  "red_flags": ["임원이 반드시 확인해야 할 우려 신호 3~6개 — 이력서·평가에서 발견된 확인 필요 항목(짧은 재직 반복, 경력 공백, 직무 전환의 설명 부재, 기대 처우 간극 등) 우선 (선택)"]
}

## 분량 (가이드가 아니라 **요구사항**이다)
- 섹션 **6~8개**, 섹션당 질문 **5~7개**. **총 24개 이상 36개 이하.**
- **총합이 24개 미만이면 실패한 출력이다.** 다 쓴 뒤 세어 보고, 모자라면 아직 안 쓴 중심 축을 더 꺼내 섹션을 늘려라.
- **직무 관련 섹션은 1개·질문 4~6개를 넘기지 말 것.** 나머지 섹션은 모두 인재상·컬쳐핏·가치관·조직 기여·리더십·위기 대응·윤리·커리어 흐름·기대치 정렬 축으로.
- **"커리어 흐름 확인"과 "기대치 정렬" 섹션은 반드시 포함하라** — 회사가 최종 결정 전에 확인해야 할 것들이다.
...
## 출력 직전 자가 점검 (하나라도 어긋나면 고쳐서 다시 써라)
1. 섹션 6개 이상인가? 2. 총 질문 24개 이상인가? 3. "커리어 흐름 확인"·"기대치 정렬" 섹션이 있는가?
4. 직무 섹션이 1개 이하인가? 5. question/followups 에 대괄호 빈칸이 하나도 없는가?`;
}
...
- 면접관이 여러 명일 수 있다(패널) — 질문하는 라벨이 여럿이면 **모두 interviewer**.
  최종 구분은 면접관/지원자 둘뿐이다. 면접관을 1번·2번으로 나누려 하지 마라.
- **지원자는 보통 한 명**이다. 자기 경험을 가장 많이 답하는 라벨을 candidate 로 본다.
- 라벨이 "P2#화자1" 처럼 접두사를 달고 있으면 P2 는 **녹음 구간 번호**다. 같은 사람이라도
  구간이 다르면 다른 라벨을 받으므로, 각 라벨을 그 구간의 대화 흐름으로 **따로** 판정하라
  (구간이 달라도 질문하는 쪽은 interviewer, 답하는 쪽은 candidate).
