...
2. **경험 깊이 (experience_depth)** — **JD 직무와 매칭되는 업무를 실제로 한 경력**의 연수 × 책임 범위 × 프로젝트 규모·난이도 (가중 0.20)
   · **핵심 원칙: 총 경력 연수가 아니라 "JD 직무와 매칭되는 업무를 한 기간"을 기준으로 본다.** 경력직이 여러 회사를 다녔다면 각 회사에서 한 일이 JD 와 일치하는지 따져, **JD 직무를 실제로 수행한 회사·기간만 깊이의 근거**로 삼는다. JD 와 무관한 회사의 재직 기간은 깊이로 합산하지 말 것.
   · 예: A법인 2년·B법인 4년·C법인 3년(총 9년)인데 **C법인에서만** JD 직무를 했다면, 깊이 판단의 핵심 근거는 **C법인 3년**이다. D법인에서 5년 내내 JD 직무를 한 후보자가 깊이로는 더 위다. 총 경력이 길면 **소폭 가점**은 가능하나(인접 도메인·일반 직무 역량), 매칭 경력이 짧으면 깊이 점수를 높게 주지 말 것.
   ...
   · 0~29: JD 매칭 업무 경력 1~2년 또는 단순 참여·보조 수준
   · 30~59: JD 매칭 업무 3~5년이나 책임 범위가 제한적, 또는 매칭 경력이 여러 회사에 짧게 흩어져 한 곳의 깊이 근거가 약함 (← 총 경력은 길어도 매칭 경력이 짧으면 여기, **70 초과 금지**)
   · 60~79: **한 회사에서** JD 매칭 업무 5~8년 + 설계·운영 책임 명확, 또는 3~5년이라도 아키텍처/리드 수준 깊은 오너십 근거를 인용 가능
   · 80~100: **단일·연속** JD 매칭 경력 8년+ 또는 대규모 시스템 설계·리드 책임이 정량·구체적으로 드러남
3. **직무 매칭도 (role_match)** — JD 주요 업무와 후보자 **과거 수행 업무(실무 경력)** 의 일치 (가중 0.15)
...
- verdict:
  · **fail** — 후보자가 그 **필수 요건을 명백히 충족하지 못함** (이력서에 충족 근거 전무). → missing 에 미충족 항목 나열. 시스템이 종합 점수를 결격 수준으로 **하드캡**한다.
