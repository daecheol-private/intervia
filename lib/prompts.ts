...
## 채용 담당자의 평가 가이드 (HR 코멘트 — 후보자 비공개) ★ 최우선 — 다른 모든 축을 덮어씀 ★
다음은 본 공고 채용 담당자가 평가 시 가장 중요하게 보라고 직접 명시한 항목이다.
**이 가이드는 6축 점수보다 우선순위가 높다.** 아래 \`focus_match\` 판정으로 종합 점수가 코드에서 강제 상향/하향(cap/floor)된다 — 6축이 아무리 좋아도 이 가이드 위반이면 점수가 깔리고, 6축이 다소 부족해도 강하게 부합하면 점수가 보장된다.
- focus_match.verdict 를 다음 중 하나로 판정하라 (가이드 부합 여부 기준):
  · **fatal_fail** — 가이드가 명시한 **필수/배제 조건을 위반** (예: "보안 도메인 없으면 최하점"인데 보안 경험 전무). → 종합 최하점·비추천 강제.
  · **fail** — 가이드 핵심 요구에 명백히 미달(치명적까진 아님). → 비추천 구간 강제.
  · **neutral** — 가이드와 무관하거나 부합·미달이 뚜렷하지 않음. → 점수 변동 없음.
  · **strong_pass** — 가이드가 강조한 항목에 **명확히 부합**. → 추천 이상 보장.
...
1. **기술 적합도 (tech_fit)** — JD 핵심 도메인·기술 직접 부합 (가중 0.30)
   ...
   · 30~59: 범용 언어·프레임워크는 부합하나 **JD 핵심 도메인 직접 경험이 없음** (← 언어만 맞고 도메인 공백이면 여기, **60 초과 금지**)
   · 60~79: JD 핵심 도메인 일부 경험 + 범용 기술 충분
   · 80~100: JD 핵심 도메인 직접 경험이 명확히 인용 가능 + 범용 기술도 부합
2. **경험 깊이 (experience_depth)** — 연차 × 책임 범위 × 프로젝트 규모·난이도 (가중 0.20)
   · 0~29: 1~2년 또는 단순 참여·보조 수준
   · 30~59: 3~5년이나 책임 범위가 제한적이거나 단일 도메인·반복 업무 위주 (← 4~5년차 풀스택이라도 설계·운영 오너십 근거가 약하면 여기, **70 초과 금지**)
   · 60~79: 5~8년 + 설계·운영 책임 명확, 또는 3~5년이라도 아키텍처/리드 수준 깊은 오너십 근거를 인용 가능
   · 80~100: 8년+ 또는 대규모 시스템 설계·리드 책임이 정량·구체적으로 드러남
3. **직무 매칭도 (role_match)** — JD 주요 업무와 후보자 과거 수행 업무의 일치 (가중 0.15)
   · 0~29: 거의 다른 직무
   · 30~59: 일부 겹치나 핵심 업무 미경험
   · 60~79: 연차·주요 업무 대체로 일치, 일부 핵심 영역 공백
   · 80~100: JD 주요 업무 대부분을 직접 수행한 이력
...
6. **성장·태도 (growth_attitude)** — **주요 도메인 외 스킬·학습·교육·자격증 노력 기준** (가중 0.10)
   · 자격증·사이드프로젝트·강의 수강·기술 블로그·오픈소스 등 **자기주도 학습의 구체적 흔적**과 선호 인재상 부합·커리어 발전 일관성을 본다.
   · 0~29: 학습·성장 흔적 전무
   · 30~59: 단편적 흔적 1~2개 (근거가 거의 없으면 50 중립)
   · 60~89: 꾸준한 학습 흔적(자격증·사이드프로젝트 등) 복수 + 커리어 발전 일관성
   · 90~100: 도메인 외까지 폭넓고 지속적인 학습 + 명확한 성장 궤적
