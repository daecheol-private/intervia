...
describe("CT-11 이력서 추출 판정", () => {
  ...
  it("CT-1111 채용포털 양식 '○○대학교(지역) 대학교(4년) 졸업' 을 정확히 읽는다", () => {
    ...
    // 캠퍼스 표기는 학교명에 붙인다(2026-07-28) — 같은 대학이라도 캠퍼스가 다르면 다른 학교로
    // 보는 통례를 따른다. 붙이는 건 캠퍼스/지역뿐이고 "(4년)"·"(박사)" 같은 학제·학위는 제외된다.
    const cases: [string, string, string | null][] = [
      ["영산대학교(부산) 대학교(4년) 졸업", "학사 졸업", "영산대학교(부산)"],
      ["강원대학교(삼척) 대학교(4년) 졸업", "학사 졸업", "강원대학교(삼척)"], // 지역명에 속아 "삼척대학교" 가 되면 안 됨
