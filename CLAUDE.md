...
npm run check:masking          # 마스킹 과검출/유출 (같은 샘플, lib/mask.ts 를 고쳤으면 필수)
```
...
`check:masking` 은 반대 방향(`lib/mask.ts`)을 잰다 — **과검출**(경력 기간·수상일·자격증 취득일이
`[생년월일]` 로 삼켜져 평가 근거가 소실)과 **유출**(생년월일 라벨 근처 날짜가 안 가려짐)을 동시에 본다.
둘은 트레이드오프라 한쪽만 재면 반드시 다른 쪽이 무너진다. **유출은 0 이어야 한다**(exit code 1).
2026-07-28 기준: 치환 30건 / 유출 0 (샘플 93건). 마스킹을 손봤으면 이 수치와 비교할 것.

⚠️ **`check:masking` 이 재는 건 생년월일뿐이다.** 전화·회사·부서·직책 **라벨**(`LABELS`)의 과검출·유출은
자동 회귀가 없다. 라벨 규칙을 건드렸으면 **수정 전 `mask.ts` 와 마스킹 결과를 전량 비교**해서 새로
노출되는 조각에 PII 가 없는지 직접 확인할 것 — `git show HEAD:lib/mask.ts` 를 임시 모듈로 뽑아
샘플 93건에 두 버전을 돌리고, 토큰이 줄어든 줄만 추려 보면 된다(2026-07-28 실측 236건 중
실질 노출은 회사 약어 1건). 라벨은 **명시 라벨**("소속회사"·"회사명" — 산문에 안 나옴, 구분자 없이 허용)과
**모호 라벨**("회사"·"직장"·"소속" — 구분자 필수)을 갈라 두었다. 모호 라벨을 느슨하게 되돌리면
자기소개서가 통째로 `[회사]` 로 사라진다.
